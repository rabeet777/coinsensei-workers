...
export class SignerService {
  ...
  private readonly signerApiKey: string;
  private readonly serviceName: string;
  private readonly timeout: number = 15000; // 15 seconds

  constructor(serviceName: string = 'gas-worker') {
   ...
   */
  async signTransaction(request: SignerRequest): Promise<SignerResponse> {
    try {
      logger.debug(
        {
          chain: request.chain,
      ...
      try {
        ...
        logger.debug(
          { endpoint: signerEndpoint, chain: request.chain },
          'Calling signer service endpoint'
        );
        ...
        if (!response.ok) {
          ...
          if (errorCode === 'UNAUTHORIZED' || response.status === 401) {
            const err: any = new Error(`Signer unauthorized: ${errorMessage}`);
            err.isRetryable = false;
            err.errorType = 'unauthorized';
            err.errorCode = 'UNAUTHORIZED';
            throw err;
          } else if (errorCode === 'VAULT_UNAVAILABLE' || errorCode === 'VAULT_ERROR') {
            const err: any = new Error(`Vault unavailable: ${errorMessage}`);
            ...
            const err: any = new Error(`Signer service error (${response.status}): ${errorMessage}`);
        ...
        if (fetchError.name === 'AbortError') {
          const err: any = new Error('Signer service timeout (15s)');
          err.isRetryable = true;
        ...
        const fetchFailureError: any = new Error(
          `Signer service connection failed: ${fetchError.message || 'Unknown error'}`
        );
