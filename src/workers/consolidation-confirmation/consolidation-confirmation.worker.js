...
export class ConsolidationConfirmationWorker {
    ...
    constructor() {
        ...
        this.runtime = new WorkerRuntime(workerIdentity('consolidation_confirmation', null));
    }
    /**
    ...
    async processBatch() {
        try {
            ...
            logger.info({ jobCount: jobs.length, workerId: this.runtime.workerId }, 'Processing consolidation confirmation batch');
            ...
            logger.error({ error: error.message, workerId: this.runtime.workerId }, 'Error in Consolidation Confirmation Worker batch processing');
        }
    }
    /**
    ...
    async confirmTronTransaction(job, chainConfig) {
        try {
            ...
            logger.error({ error: error.message, jobId: job.id, txHash: job.tx_hash }, 'Error confirming TRON consolidation transaction');
    ...
    async confirmEvmTransaction(job, chainConfig) {
        try {
            ...
            logger.error({ error: error.message, jobId: job.id, txHash: job.tx_hash }, 'Error confirming EVM consolidation transaction');
            // Do not update job - leave in confirming state for retry
        }
    }
    /**
    ...
    async finalizeSuccess(job, chainConfig, confirmations, blockNumber, gasUsed, gasPrice) {
        try {
            ...
            const { error: queueError } = await this.supabase
                ...
                .eq('id', job.id);
            if (queueError) {
                logger.error({ error: queueError.message, jobId: job.id }, 'Failed to update consolidation_queue status to confirmed');
                return;
            }
            ...
            logger.error({ error: error.message, jobId: job.id, txHash: job.tx_hash }, 'Error finalizing successful consolidation');
        }
    }
    /**
    ...
    async finalizeFailure(job, chainConfig, reason) {
        try {
            ...
            const { error: queueError } = await this.supabase
                ...
                .eq('id', job.id);
            if (queueError) {
                logger.error({ error: queueError.message, jobId: job.id }, 'Failed to update consolidation_queue status to failed');
                return;
            }
            ...
            logger.error({ error: error.message, jobId: job.id, txHash: job.tx_hash }, 'Error finalizing failed consolidation');
        }
    }
    /**
     * Release consolidation lock on wallet_balances
     */
    async releaseConsolidationLock(job, success) {
        try {
            const updates = {
            ...
            logger.debug({ walletBalanceId: job.wallet_balance_id, success }, 'Released consolidation lock at wallet level');
        }
        catch (error) {
            logger.error({ error: error.message, walletBalanceId: job.wallet_balance_id }, 'Error releasing consolidation lock');
        }
    }
    /**
    ...
    async start() {
        ...
        while (this.isRunning) {
            ...
            try {
                ...
                logger.error({ error: error.message }, 'Error in Consolidation Confirmation Worker loop');
                await this.runtime.logExecution({
                    executionType: 'cycle',
                    status: 'fail',
                    durationMs: Date.now() - cycleStart,
...
//# sourceMappingURL=consolidation-confirmation.worker.js.map