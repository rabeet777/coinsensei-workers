...
export class BscConsolidationWorker {
    ...
    async executeConsolidation(job) {
        ...
        const txHash = signerResult.tx_hash || signerResult.txHash || signerResult.transactionHash;
        if (!txHash) {
    ...
    async loadAssetInfo(walletBalanceId) {
        ...
        if (wbError || !walletBalance) {
            logger.error({ error: wbError?.message, walletBalanceId }, 'Error loading wallet balance');
        ...
        if (aocError || !assetOnChain) {
            logger.error({ error: aocError?.message, assetOnChainId: walletBalance.asset_on_chain_id }, 'Error loading asset info');
            throw new Error('Asset info not found');
        }
        return assetOnChain;
    }
    ...
    async start() {
        if (this.isRunning || !this.runtime) {
            if (!this.runtime)
                ...
                logger.warn('BSC Consolidation Worker already running');
            return;
        }
        this.isRunning = true;
        ...
        while (this.isRunning) {
            ...
            try {
                ...
                logger.error({ error: error.message }, 'Error in BSC consolidation worker loop');
                await this.runtime.logExecution({
                    executionType: 'cycle',
                    status: 'fail',
                    durationMs: Date.now() - cycleStart,
...
//# sourceMappingURL=bsc-consolidation.worker.js.map