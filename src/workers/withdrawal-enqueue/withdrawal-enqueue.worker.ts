...
interface WithdrawalRequest {
  id: string;
  user_id: string;
  amount?: number; // May be named differently
  amount_human?: number; // Alternative field name
  to_address: string;
  asset_id?: string; // May be missing
  asset_on_chain_id?: string; // Alternative field name
  chain_id: string;
  status: string;
  created_at: string;
...
export class WithdrawalEnqueueWorker {
  ...
  constructor() {
    ...
    this.runtime = new WorkerRuntime(
      workerIdentity('withdrawal_enqueue', null)
    );
  }
  ...
  async processBatch(): Promise<void> {
    try {
      ...
      logger.info(
        { count: requests.length, workerId: this.runtime.workerId },
        'Processing approved withdrawal requests'
      );
      ...
      logger.error(
        ...
        'Error in Withdrawal Enqueue Worker batch processing'
      );
    }
  }
   ...
   */
  private async pickApprovedRequests(): Promise<WithdrawalRequest[]> {
   ...
   */
  private async processRequest(request: WithdrawalRequest): Promise<void> {
    try {
      ...
      let assetOnChain: AssetOnChain | null = null;
      ...
      if (request.asset_on_chain_id) {
        ...
        if (!assetOnChain) {
          throw new Error(
            `Asset on chain not found: asset_on_chain_id=${request.asset_on_chain_id}`
          );
        }
        ...
        if (!assetOnChain) {
          throw new Error(
            `Asset not found on chain: asset_id=${request.asset_id}, chain_id=${request.chain_id}`
          );
        }
      } else {
        logger.error(
        ...
        throw new Error(
          `Withdrawal request ${request.id} is missing required field: asset_id or asset_on_chain_id`
        );
      }
      ...
      if (amountHuman === undefined || amountHuman === null) {
        ...
        throw new Error(
          `Withdrawal request ${request.id} is missing required field: amount or amount_human`
        );
      }

      if (typeof amountHuman !== 'number' || isNaN(amountHuman)) {
        throw new Error(
          `Withdrawal request ${request.id} has invalid amount: ${amountHuman} (type: ${typeof amountHuman})`
        );
      }
      ...
      if (assetOnChain.decimals === undefined || assetOnChain.decimals === null) {
        throw new Error(
          `Asset on chain ${assetOnChain.id} has invalid decimals: ${assetOnChain.decimals}`
        );
      }
      ...
      if (updateRequestError) {
        logger.error(
          { error: updateRequestError.message, requestId: request.id },
          'Failed to update withdrawal_requests status'
        );
      ...
      if (updateWalletError) {
        logger.warn(
          { error: updateWalletError.message, walletId: hotWallet.id },
          'Failed to update hot wallet last_used_at'
        );
   ...
   */
  private async loadChainConfig(chainId: string): Promise<ChainConfig | null> {
   ...
   */
  private async loadAssetOnChain(
    assetId: string,
    chainId: string
  ): Promise<AssetOnChain | null> {
    const { data, error } = await this.supabase
      .from('asset_on_chain')
      .select('*')
      .eq('asset_id', assetId)
   ...
   */
  private async loadAssetOnChainById(assetOnChainId: string): Promise<AssetOnChain | null> {
    const { data, error } = await this.supabase
      .from('asset_on_chain')
      .select('*')
    ...
    if (error) {
      logger.error(
        { error: error.message, assetOnChainId },
        'Error loading asset on chain by ID'
      );
      return null;
    }
   ...
   */
  private async selectHotWallet(chainId: string): Promise<OperationWallet | null> {
    const { data, error } = await this.supabase
      .from('operation_wallet_addresses')
      .select('*')
      .eq('chain_id', chainId)
   ...
   */
  private calculateRawAmount(humanAmount: number, decimals: number): string {
    // Validate inputs
    ...
    try {
      ...
      throw new Error(
        `Failed to calculate raw amount: humanAmount=${humanAmount}, decimals=${decimals}, error=${error.message}`
      );
    }
  }
  ...
  async start(): Promise<void> {
    ...
    while (this.isRunning) {
      ...
      try {
        ...
        logger.error(
          { error: error.message },
          'Error in Withdrawal Enqueue Worker loop'
        );
        await this.runtime.logExecution({
          executionType: 'cycle',
          status: 'fail',
