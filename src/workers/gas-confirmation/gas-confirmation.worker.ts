...
import type { GasTopupJob } from '../../types/gas-topup-queue.js';
import {
  WorkerRuntime,
  workerIdentity,
...
export class GasConfirmationWorker {
  ...
  private readonly BATCH_SIZE = 10; // Process up to 10 jobs per batch
  private chains: Map<string, ChainConfig> = new Map();
  private tronClients: Map<string, any> = new Map();
  private bscClients: Map<string, any> = new Map();

  constructor() {
    this.supabase = getSupabaseClient();
    this.runtime = new WorkerRuntime(
      workerIdentity('gas_topup_confirmation', null)
    );
  }
  ...
  async processBatch(): Promise<void> {
    try {
      ...
      logger.info(
        ...
        'Processing gas confirmation batch'
      );
      ...
      logger.error(
        { error: error.message },
        'Error processing confirmation batch'
      );
    }
  }
   ...
   */
  private async pickConfirmingJobs(): Promise<GasTopupJob[]> {
    try {
   ...
   */
  private async confirmJob(job: GasTopupJob): Promise<void> {
    if (!job.tx_hash) {
    ...
    if (chainConfig.name.toLowerCase() === 'tron') {
      ...
      logger.warn(
        { jobId: job.id, chain: chainConfig.name },
        'Unsupported chain type for confirmation'
      );
    }
  }
  ...
  private async confirmTronTransaction(
    ...
    try {
      ...
      logger.error(
        { error: error.message, jobId: job.id, txHash: job.tx_hash },
        'Error confirming TRON transaction'
      );
   ...
   */
  private async confirmBscTransaction(
    job: GasTopupJob,
    chainConfig: ChainConfig
  ): Promise<void> {
    try {
    ...
    try {
      ...
      if (queueError) {
        logger.error(
          ...
          'Failed to update gas_topup_queue status to confirmed'
        );
        return;
      }
  ...
  private async finalizeFailure(
    ...
    try {
      ...
      if (queueError) {
        logger.error(
          { error: queueError.message, jobId: job.id },
          'Failed to update gas_topup_queue status to failed'
        );
        return;
      }
   ...
   */
  private async releaseGasLock(job: GasTopupJob, success: boolean): Promise<void> {
    try {
      ...
      if (!gasAssetOnChain) {
        logger.warn(
          { jobId: job.id, gasAssetId: job.gas_asset_id, chainId: job.chain_id },
          'Could not resolve asset_on_chain_id for gas lock release'
        );
        return;
      }
      ...
      if (error) {
        logger.error(
          { error: error.message, jobId: job.id, walletId: job.wallet_id },
          'Failed to release gas lock'
        );
      } else {
        logger.debug(
          { jobId: job.id, walletId: job.wallet_id, success },
          'Released gas lock and updated wallet state'
        );
      }
    } catch (error: any) {
      logger.error(
  ...
  async start(): Promise<void> {
    ...
    while (this.isRunning) {
      ...
      try {
        ...
        logger.error(
          { error: error.message },
          'Error in gas confirmation worker loop'
        );
        await this.runtime.logExecution({
          executionType: 'cycle',
          status: 'fail',
