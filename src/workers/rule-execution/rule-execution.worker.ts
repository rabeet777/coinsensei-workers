...
interface GasTopupRule {
  ...
  chain_id: string;
  gas_asset_id: string;
  threshold_human: string; // The topup amount (how much gas to add)
  threshold_raw: string; // Raw topup amount
  comparison_operator: string;
  priority: number;
  is_active: boolean;
  metadata: any; // Optional: operation_wallet_address_id, custom topup amounts
}
...
export class RuleExecutionWorker {
  ...
  async processBatch(): Promise<void> {
    try {
      ...
      logger.info(
        ...
        'Processing batch for rule execution'
      );
      ...
      logger.info(
        ...
        'Rule execution batch complete'
      );
    } catch (error: any) {
      logger.error(
        { error: error.message, stack: error.stack },
        'Error processing rule execution batch'
      );
    ...
    try {
      ...
      if (userWalletError) {
        logger.error(
          ...
          'Failed to load user wallet addresses for rule execution'
        );
        return [];
      }
      ...
      if (selectError) {
        logger.error(
          ...
          'Failed to select wallet balances for rule execution'
        );
        return [];
      }
      ...
      const userWalletRows = availableRows.filter((row) =>
        userIdsSet.has(row.wallet_id)
      );

      if (userWalletRows.length !== availableRows.length) {
        const operationWalletRows = availableRows.filter(
          (row) => !userIdsSet.has(row.wallet_id)
        );
        logger.error(
          {
      ...
      if (lockError) {
        logger.error(
          ...
          'Failed to lock rows for rule execution'
        );
        return [];
      }
      ...
      logger.debug(
        ...
        'Locked wallet balance rows for rule execution (user wallets only)'
      );
  ...
  private async processWalletBalance(row: WalletBalance): Promise<void> {
    ...
    try {
      ...
      if (nativeError || !nativeGasAsset) {
        ...
        const { data: nativeGasBalance, error: gasBalanceError } =
          await this.supabase
            .from('wallet_balances')
            .select('*')
        ...
        if (gasBalanceError) {
          ...
          const gasResult = await this.executeGasTopupRules(
            nativeGasBalance,
            {
          ...
          if (gasResult.matched) {
            ...
            const consolidationResult = await this.executeConsolidationRules(
              row,
              assetOnChain
            );
            ...
            await this.finalizeWalletBalance(
              row.id,
              needsConsolidation, // Set if rule matched
              consolidationPriority,
              true, // needs_gas=true (blocks consolidation)
              gasPriority
            );

            // FIX 2: Update native gas row ONLY (not all rows for wallet)
            await this.updateNativeGasFlags(
              row.wallet_id,
              nativeGasAsset.id,
              true, // needs_gas=true
              gasPriority
      ...
      await this.finalizeWalletBalance(
        ...
        needsConsolidation,
        consolidationPriority,
        false, // needs_gas already set on native row
        0
      );
   ...
   */
  private async executeConsolidationRules(
    walletBalance: WalletBalance,
    assetOnChain: AssetOnChain
  ): Promise<{ matched: boolean; priority: number }> {
    const startTime = Date.now();
    ...
    try {
      ...
      for (const rule of rules) {
        ...
        const result = this.evaluateCondition(
          walletBalance.on_chain_balance_human,
          rule.comparison_operator,
          rule.threshold_human
        );

        // ALWAYS log rule evaluation
        await this.logConsolidationRuleExecution(
          rule.id,
   ...
   */
  private async executeGasTopupRules(
    walletBalance: WalletBalance,
    assetOnChain: AssetOnChain
  ): Promise<{ matched: boolean; priority: number }> {
    const startTime = Date.now();
    ...
    try {
      ...
      for (const rule of rules) {
        ...
        // ALWAYS log rule evaluation
        await this.logGasTopupRuleExecution(
          rule.id,
          walletBalance.wallet_id,
          result,
          {
  ...
  private evaluateCondition(
    ...
    if (isNaN(balanceNum) || isNaN(thresholdNum)) {
      logger.warn(
        { balance: balanceStr, threshold, operator },
        'Invalid number in condition evaluation'
      );
      return false;
    }
    ...
    switch (operator) {
      ...
      default:
        logger.warn(
          { operator },
          'Unknown comparison operator, defaulting to false'
        );
        result = false;
    }
   ...
   */
  private async logConsolidationRuleExecution(
    ruleId: string,
    walletId: string,
    ...
    if (error) {
      logger.error(
        { error: error.message, ruleId, walletId },
        'Failed to log consolidation rule execution'
      );
    }
  }
   ...
   */
  private async logGasTopupRuleExecution(
    ruleId: string,
    walletId: string,
    executionResult: boolean,
    executionData: any,
    ...
    if (error) {
      logger.error(
        { error: error.message, ruleId, walletId },
        'Failed to log gas top-up rule execution'
      );
    }
  }
  ...
  private async enqueueConsolidation(
    ...
    try {
      ...
      const operationWalletAddressId = await this.selectHotWalletAddress(
        assetOnChain.chain_id
      );

      if (!operationWalletAddressId) {
        throw new Error(
          `No active hot wallet address found for chain ${assetOnChain.chain_id}`
        );
      }
      ...
      if (error) {
        ...
        if (error.code === '23505') {
          logger.debug(
            { walletBalanceId: walletBalance.id },
            'Consolidation queue insert conflict, skipping'
          );
          return;
        }
      ...
      logger.error(
        { error: error.message, walletBalanceId: walletBalance.id },
        'Error enqueueing consolidation'
      );
      throw error;
    }
  }
  ...
  private async enqueueGasTopup(
    ...
    try {
      ...
      const topupAmountHuman = 
        rule.metadata?.topup_amount_human || 
        String(rule.threshold_human);
      
      const topupAmountRaw = 
        rule.metadata?.topup_amount_raw || 
        rule.threshold_raw || 
        null;

      // Select gas operation wallet address (chain-matched with fallback)
      const operationWalletAddressId = await this.selectGasWalletAddress(
        assetOnChain.chain_id,
        rule.metadata?.operation_wallet_address_id
      );

      if (!operationWalletAddressId) {
        throw new Error(
          `No active gas wallet address (or hot wallet fallback) found for chain ${assetOnChain.chain_id}`
        );
      }
      ...
      if (error) {
        ...
        if (error.code === '23505') {
          logger.debug(
            { walletBalanceId: walletBalance.id },
            'Gas top-up queue insert conflict, skipping'
          );
          return;
        }
   ...
   */
  private async selectHotWalletAddress(chainId: string): Promise<string | null> {
    try {
      ...
      if (error) {
        logger.error(
          ...
          'Error selecting hot wallet address'
        );
        return null;
      }
      ...
      if (!operationWalletAddress) {
        logger.error(
          { chainId },
          'No active hot wallet address found for chain'
        );
        return null;
      }
   ...
   * Select gas operation wallet address (chain-matched with fallback)
   */
  private async selectGasWalletAddress(
    chainId: string,
    preferredWalletAddressId?: string
  ): Promise<string | null> {
    try {
      ...
      if (preferredWalletAddressId) {
        ...
        if (preferredWallet) {
          ...
          logger.debug(
            { operationWalletAddressId: preferredWallet.id, chainId },
            'Using preferred gas wallet address from rule metadata'
          );
      ...
      if (gasWallet) {
        ...
        logger.debug(
          { operationWalletAddressId: gasWallet.id, chainId },
          'Selected gas-role wallet address'
        );
      ...
      if (hotWallet) {
        ...
        logger.debug(
          { operationWalletAddressId: hotWallet.id, chainId },
          'Selected hot wallet address as gas fallback'
        );
      ...
      logger.error(
        { chainId },
        'No active gas or hot wallet address found for chain'
      );
      ...
      logger.error(
        { error: error.message, chainId },
        'Error selecting gas wallet address'
      );
      return null;
    }
  }
   ...
   */
  private async updateWalletAddressUsage(walletAddressId: string): Promise<void> {
    try {
      const { error } = await this.supabase
        .from('operation_wallet_addresses')
      ...
      if (error) {
        logger.debug(
          { error: error.message, walletAddressId },
   ...
   */
  private async updateNativeGasFlags(
    walletId: string,
    nativeAssetOnChainId: string,
    needsGas: boolean,
    gasPriority: number
  ): Promise<void> {
    try {
      ...
      if (!userWallet) {
        logger.warn(
          { walletId, nativeAssetOnChainId },
          'Skipping gas flag update - wallet is not a user wallet (operation wallet detected)'
        );
        return;
      }
      ...
      logger.error(
        ...
        'Error updating native gas flags'
      );
    }
  }
   ...
   */
  private async finalizeWalletBalance(
    ...
    if (loadError || !walletBalance) {
      throw new Error(
        `Failed to load wallet balance for finalization: ${loadError?.message || 'not found'}`
      );
    }
  ...
  async start(): Promise<void> {
    ...
    logger.info(
      { workerId: this.runtime.workerId },
      'Starting rule execution worker loop'
    );
    ...
    while (this.isRunning) {
      ...
      try {
        ...
        logger.error(
          { error: error.message, stack: error.stack },
          'Error in rule execution worker loop'
        );
        await this.runtime.logExecution({
          executionType: 'cycle',
          status: 'fail',
  ...
  stop(): void {
    logger.info(
      { workerId: this.runtime.workerId },
      'Stopping rule execution worker'
    );
