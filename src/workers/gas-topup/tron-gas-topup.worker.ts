...
export class TronGasTopupWorker {
  ...
  private readonly CONFIRMATION_BLOCKS = 1; // Configurable
  private readonly FEE_LIMIT = 2000000; // 2 TRX in SUN
  ...
  async processBatch(): Promise<void> {
    ...
    try {
      ...
      logger.debug(
        ...
        'Starting job processing with gas lock'
      );
      await this.processJobWithGasLock(job);
    } catch (error: any) {
      logger.error(
        { error: error.message, jobId: job?.id },
        'Error processing TRON gas top-up'
      );
    }
  }
  ...
  private async processJobWithGasLock(job: GasTopupJob): Promise<void> {
    ...
    try {
      ...
      if (job.status === 'confirming') {
        logger.debug(
          { jobId: job.id, status: job.status, txHash: job.tx_hash },
          'Job in confirming state - skipping gas lock acquisition, confirmation worker handles'
        );
      ...
      logger.debug(
        { jobId: job.id, walletId: job.wallet_id, gasAssetId: job.gas_asset_id },
        'Attempting to acquire gas lock'
      );
      ...
      logger.debug(
        { jobId: job.id, walletId: job.wallet_id },
        'Gas lock acquired, processing job'
      );
      ...
      logger.debug(
        { jobId: job.id },
        'Job processed successfully - gas lock will be released by confirmation worker'
      );
    } catch (error: any) {
      logger.error(
        { error: error.message, jobId: job.id },
  ...
  private async acquireGasLock(
    ...
    try {
      ...
      if (checkError) {
        logger.error(
          { error: checkError.message, walletId, assetOnChainId: gasAssetOnChain.id },
          'Error checking wallet_balances row'
        );
        return false;
      }
      ...
      if (error) {
        logger.error(
          { error: error.message, walletId, assetOnChainId: gasAssetOnChain.id },
          'Error updating wallet_balances for gas lock'
        );
        return false;
      }
    ...
    try {
      ...
      if (job.tx_hash && job.status !== 'failed') {
        logger.info(
          ...
          'Transaction already broadcasted - confirmation worker will handle outcome'
        );
      ...
      if (job.status === 'pending' || job.status === 'processing') {
        ...
        logger.warn(
          ...
          'Job in unexpected state for processing'
        );
      }
    } catch (error: any) {
      await this.handleJobError(job, error);
  ...
  private async executeNewJob(job: GasTopupJob): Promise<void> {
    ...
    logger.debug(
      { jobId: job.id, operationWalletAddressId: job.operation_wallet_address_id },
      'Loading funding wallet address'
    );
    ...
    if (!fundingWallet) {
      ...
      logger.error(
        { jobId: job.id, operationWalletAddressId: job.operation_wallet_address_id },
        'Funding wallet address not found'
      );
    ...
    logger.debug(
      { jobId: job.id, fundingAddress: fundingWallet.address },
      'Funding wallet loaded'
    );

    // D) Load target wallet address
    logger.debug(
      { jobId: job.id, walletId: job.wallet_id },
      'Loading target wallet address'
    );
    ...
    logger.debug(
      { jobId: job.id, address: fundingWallet.address, amount: job.topup_amount_raw },
      'Validating funding wallet balance'
    );
    ...
    logger.debug(
      { jobId: job.id, from: fundingWallet.address, to: targetWallet.address },
      'Preparing TRON transaction intent'
    );
    // Build transaction intent (from, to, amount_sun)
    const txIntent = await this.buildTransactionIntent(
      fundingWallet.address,
      targetWallet.address,
      job.topup_amount_raw || job.topup_amount_human
    );
    ...
    try {
      ...
      const signerResponse: any = signerResult;
      ...
      logger.info(
        { jobId: job.id, txHash },
        'Transaction built, signed, and broadcasted successfully by signer service'
      );
      ...
      if (
        signerError.errorCode === 'TAPOS_ERROR' ||
        ...
        const taposError: any = new Error(`TAPOS_ERROR: ${signerError.message}`);
  ...
  private async loadTargetWalletAddress(
    walletId: string,
    chainId: string
  ): Promise<{ address: string; uid: string; wallet_group_id: string; derivation_index: number } | null> {
    const { data, error } = await this.supabase
  ...
  private async validateFundingBalance(
    ...
    logger.debug(
      ...
      'Balance validation delegated to signer service'
    );
  }
   ...
   */
  private async buildTransactionIntent(
    from: string,
    to: string,
    amount: string
  ): Promise<{ type: string; from: string; to: string; amount_sun: string }> {
    try {
      ...
      logger.debug(
        { from, to, amount_sun: amountSun },
        'Built TRON transaction intent'
      );
      ...
      logger.error(
        ...
        'Failed to build TRON transaction intent'
      );
   ...
   */
  private classifyTronError(error: any): void {
   ...
   */
  private async updateWalletGasState(
    walletId: string,
    chainId: string,
    needsGas: boolean
  ): Promise<void> {
    try {
      ...
      if (error) {
        logger.error(
          { error: error.message, walletId },
          'Failed to update wallet gas state'
        );
      } else {
        logger.info(
          { walletId, needsGas },
          'Updated wallet gas state'
        );
      }
    } catch (error: any) {
      logger.error(
        { error: error.message, walletId },
        'Error updating wallet gas state'
      );
    }
  }
   ...
   */
  private async writeAuditRecord(job: GasTopupJob): Promise<void> {
    try {
  ...
  async start(): Promise<void> {
    if (this.isRunning || !this.runtime) {
      ...
      else if (this.isRunning) logger.warn('TRON Gas Top-Up Worker already running');
      return;
    }
    ...
    while (this.isRunning) {
      ...
      try {
        ...
        logger.error(
          { error: error.message },
          'Error in TRON gas top-up worker loop'
        );
        await this.runtime.logExecution({
          executionType: 'cycle',
          status: 'fail',
