...
export class BscGasTopupWorker {
    ...
    MAX_RETRIES = 8;
    CONFIRMATION_BLOCKS = 3; // Configurable
    GAS_LIMIT = 21000; // Standard ETH transfer
    MAX_GAS_PRICE_GWEI = 20; // Cap gas price
    GAS_PRICE_BUMP_PERCENT = 15; // For replacements
    ...
    async processBatch() {
        try {
            ...
            logger.error({ error: error.message }, 'Error processing BSC gas top-up');
        }
    }
    /**
    ...
    async processJobWithGasLock(job) {
        ...
        try {
            ...
            if (!gasLockAcquired) {
                logger.debug({ jobId: job.id, walletId: job.wallet_id }, 'Could not acquire gas lock - another worker owns it, skipping');
     ...
     */
    async acquireEvmFunderLock(fundingAddress) {
        try {
            const { error } = await this.supabase.rpc('lock_evm_funder', {
                key: fundingAddress.toLowerCase(),
            });
            if (error) {
                logger.error({ error: error.message, fundingAddress }, 'Failed to acquire EVM funder advisory lock');
            ...
            logger.error({ error: error.message, fundingAddress }, 'Error acquiring EVM funder lock');
            throw error;
        }
    }
    /**
     ...
     */
    async releaseEvmFunderLock(fundingAddress) {
        try {
            const { error } = await this.supabase.rpc('unlock_evm_funder', {
                key: fundingAddress.toLowerCase(),
            });
            if (error) {
                logger.error({ error: error.message, fundingAddress }, 'Failed to release EVM funder advisory lock');
            ...
            logger.error({ error: error.message, fundingAddress }, 'Error releasing EVM funder lock');
            // Don't throw - best effort release
        }
    }
    /**
    ...
    async processJob(job) {
        try {
            ...
            if (job.tx_hash && job.status !== 'failed') {
                logger.info({ jobId: job.id, txHash: job.tx_hash, status: job.status }, 'Transaction already exists - resuming confirmation only');
            ...
            if (job.status === 'pending' || job.status === 'processing') {
                ...
                logger.warn({ jobId: job.id, status: job.status }, 'Job in unexpected state');
            }
        }
        catch (error) {
            await this.handleJobError(job, error);
     ...
     */
    async executeWithNonceLock(job, fundingWallet) {
        ...
        if (gasPrice > maxGasPrice) {
            const error = new Error(`Gas price too high: ${ethers.formatUnits(gasPrice, 'gwei')} > ${this.MAX_GAS_PRICE_GWEI} Gwei`);
            error.isRetryable = true;
        ...
        const unsignedTx = await this.buildUnsignedTransaction(fundingWallet.address, targetWallet, job.topup_amount_raw || ethers.parseEther(job.topup_amount_human).toString(), job, gasPrice);
    ...
    async validateFundingBalance(address, requiredAmount) {
        ...
        if (balance < required) {
            throw new Error(`Insufficient funding balance: ${balance} < ${required} (Wei)`);
        }
        logger.debug({ address, balance: balance.toString(), required: required.toString() }, 'Funding wallet balance validated');
    }
    /**
     ...
     */
    async buildUnsignedTransaction(from, to, amount, job, gasPrice) {
        try {
            ...
            logger.error({ error: error.message, from, to, amount }, 'Failed to build BSC transaction');
     ...
     */
    async signTransaction(fundingWallet, unsignedTx) {
        try {
     ...
     */
    async broadcastTransactionWithReplacement(signedTx, job, originalTx) {
        try {
            const tx = await this.provider.broadcastTransaction(signedTx);
            logger.info({ txHash: tx.hash, nonce: originalTx.nonce }, 'BSC transaction broadcasted successfully');
            return tx.hash;
        }
        catch (error) {
            ...
            if (errorMsg.includes('replacement') && errorMsg.includes('underpriced')) {
                ...
                if (bumpedGasPrice > maxGasPrice) {
                    const err = new Error(`Gas price bump exceeds cap: ${ethers.formatUnits(bumpedGasPrice, 'gwei')} > ${this.MAX_GAS_PRICE_GWEI} Gwei`);
            ...
            logger.error({ error: error.message, jobId: job.id }, 'Failed to broadcast BSC transaction');
            this.classifyBscError(error);
            throw error;
        }
    }
    ...
    async confirmTransaction(job) {
        ...
        try {
            ...
            if (!receipt) {
                logger.debug({ jobId: job.id, txHash: job.tx_hash }, 'Transaction not yet mined');
            ...
            if (receipt.status === 0) {
                const err = new Error('Transaction failed on-chain (status = 0)');
                err.isRetryable = false;
     ...
     * Write audit record
     */
    async writeAuditRecord(job, receipt) {
        try {
            const fundingWallet = await this.loadOperationWalletAddress(job.operation_wallet_address_id);
    ...
    async start() {
        if (this.isRunning || !this.runtime) {
            if (!this.runtime)
                ...
                logger.warn('BSC Gas Top-Up Worker already running');
            return;
        }
        this.isRunning = true;
        ...
        while (this.isRunning) {
            ...
            try {
                ...
                logger.error({ error: error.message }, 'Error in BSC gas top-up worker loop');
                await this.runtime.logExecution({
                    executionType: 'cycle',
                    status: 'fail',
                    durationMs: Date.now() - cycleStart,
...
//# sourceMappingURL=bsc-gas-topup.worker.js.map