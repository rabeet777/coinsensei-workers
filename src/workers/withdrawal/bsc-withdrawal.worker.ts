...
export class BscWithdrawalWorker {
  ...
  async processBatch(): Promise<void> {
    try {
      ...
      logger.error(
        { error: error.message },
        'Error in BSC withdrawal batch processing'
      );
    }
  }
  ...
  private async processJob(job: WithdrawalJob): Promise<void> {
    ...
    try {
      ...
      logger.debug(
        {
          ...
          contractAddress: (txIntent as any).contractAddress || 'N/A',
        },
      ...
      const txHash =
        ...
        (signerResult as any).transactionHash;
  ...
  async start(): Promise<void> {
    if (this.isRunning || !this.runtime) {
      ...
      else if (this.isRunning) logger.warn('BSC Withdrawal Worker already running');
      return;
    }
    ...
    while (this.isRunning) {
      ...
      try {
        ...
        logger.error(
          { error: error.message },
          'Error in BSC withdrawal worker loop'
        );
        await this.runtime.logExecution({
          executionType: 'cycle',
          status: 'fail',
